...
--QUESTION 1: Write a SQL query that returns four columns from the Products table: product_code, product_name, list_price, and discount_percent. Then, run this statement to make sure it works correctly.
...
--QUESTION 2: Write a SQL query that returns one column from the Customers table named full_name that joins the last_name and first_name columns. 
--Format this column with the last name, a comma, a space, and the first name.
--Sort the result set by last name in ascending sequence. Return only the customers whose last name begins with letters from M to Z.
SET ECHO ON;
SELECT last_name ||  ', ' || first_name "Customer"
FROM customers
WHERE last_name >= 'M%' 
ORDER BY last_name;

--QUESTION 3:  Write a SELECT statement that returns these columns from the Products table: product_name, list_price, date_added
...
--QUESTION 4:  Write a SELECT statement that returns these column names and data from the Products table: product_name, list_price, discount_price, discount_amount, _discount_price
...
SELECT product_name, list_price, discount_percent,
    list_price * discount_percent * .01 AS discount_amount,
    list_price - (list_price * discount_percent * .01) AS discount_price
FROM products
WHERE ROWNUM <= 5
ORDER BY discount_price DESC;

--QUESTION 5:  Write a SELECT statement that returns these column names and data from the Order_Items table: item_id, item_price, discount_amount, quantity, price_total, discount_total, item_total
...
--QUESTION 6:  Write a SELECT statement that returns these columns from the Orders table: order_id, order_date, ship_date
...
--QUESTION 7: Write a SELECT statement that uses the SYSDATE function to create a row with these columns: today_unformatted, today_formatted
...
--QUESTION 8: Write a SELECT statement that creates a row with these columns: price, tax_rate, tax_amount, total
--To calculate the fourth column, add the expressions you used for the first and third columns.
--Use a FROM clause that specifies the Dual table.
SET ECHO ON;
SELECT 100 AS price,
.07 AS tax_rate,
100 * 0.7 AS tax_amount,
100 + (100*0.7) AS total
FROM Dual;

--QUESTION 9: Write a SELECT statement that joins the Categories table to the Products table and returns these columns: category_name, product_name, list_price.
...
--QUESTION 10:  Write a SELECT statement that joins the Customers table to the Addresses table and returns these columns: first_name, last_name, line1, city, state, zip_code.
...
--QUESTION 11:  Write a SELECT statement that joins the Customers table to the Addresses table and returns these columns: first_name, last_name, line1, city, state, zip_code.
...
--QUESTION 12:  Write a SELECT statement that joins the Customers, Orders, Order_Items, and Products tables. 
...
SET ECHO ON;
SELECT  last_name ||  ', ' || first_name "Customer", order_date "Order Date", product_name "Product", item_price "Price", discount_amount "Discount Amount", quantity "Quantity"
FROM customers c, orders o, order_items oi, products p
WHERE c.customer_id = o.customer_id AND oi.product_id = p.product_id
